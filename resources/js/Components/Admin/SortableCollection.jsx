...
export default function SortableCollection({ items = [], onChange, renderItem, getKey = (item, index) => item.id || index, onRemove, label = 'item', gridClass = 'space-y-4', itemClass = '' }) {
    ...
    return <div className={gridClass}>{items.map((item, index) => {
        ...
        return <article key={key} draggable onDragStart={(event) => { if (event.target.closest('button, a, input, select, textarea')) { event.preventDefault(); return; } setDragging(key); event.dataTransfer.effectAllowed = 'move'; }} onDragEnd={() => setDragging(null)} onDragOver={(event) => dragOver(event, index)} className={`cursor-grab overflow-hidden rounded-xl border border-line bg-white transition-[opacity,box-shadow,border-color] duration-150 active:cursor-grabbing ${active ? 'border-brand opacity-65 shadow-lg' : 'shadow-sm'} ${itemClass}`}>
            <div className="flex h-9 items-center justify-between gap-2 border-b border-line bg-surface px-2 text-xs text-muted">
                <div className="flex items-center gap-2"><button type="button" aria-label={`Reordenar ${label} ${index + 1}`} title="Arraste para reordenar" onKeyDown={(event) => { if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') { event.preventDefault(); move(index, index - 1); } if (event.key === 'ArrowRight' || event.key === 'ArrowDown') { event.preventDefault(); move(index, index + 1); } }} className="cursor-grab rounded px-2 py-1 text-base leading-none transition-colors hover:bg-line active:cursor-grabbing">&#8942;&#8942;</button><span aria-label={`Posi\u00e7\u00e3o ${index + 1}`} className="grid size-5 place-items-center rounded-full bg-white font-medium text-ink">{index + 1}</span></div>
