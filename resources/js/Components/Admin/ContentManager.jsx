                    ...
                    return (
                        <article key={row.id || row._key || index} className="rounded-xl border border-line bg-surface p-4">
                            <div className="flex items-start justify-between gap-4">
                                ...
                                <button type="button" className="rounded-full bg-brand px-3 py-1 text-xs font-medium text-white transition-colors hover:bg-brand-dark" onClick={() => updateRows(rows.filter((_, rowIndex) => rowIndex !== index))}>Remover período</button>
                            </div>
...
function PendingMediaPreview({ file, onPrevious, onNext, onRemove, previousDisabled, nextDisabled }) {
    ...
    return <div className="overflow-hidden rounded-lg border border-line bg-white">
        ...
        <div className="flex items-center justify-between gap-2 p-2 text-xs"><button type="button" disabled={previousDisabled} onClick={onPrevious} className="rounded px-2 py-1 transition-colors hover:bg-line disabled:cursor-not-allowed disabled:opacity-30">Anterior</button><button type="button" disabled={nextDisabled} onClick={onNext} className="rounded px-2 py-1 transition-colors hover:bg-line disabled:cursor-not-allowed disabled:opacity-30">Próxima</button><button type="button" className="grid size-6 place-items-center rounded-full bg-brand text-sm font-medium text-white transition-colors hover:bg-brand-dark disabled:cursor-not-allowed disabled:opacity-60" onClick={onRemove}>×</button></div>
    </div>;
}
