...
export default function Form({ item, options }) {
    ...
    return <AdminLayout title={editing ? `Editar ${text.condominium}` : `Novo ${text.condominium}`}>
        <form onSubmit={submit} className="space-y-8">
            ...
            <section className="grid gap-5 rounded-card bg-white p-6 shadow-card tablet:grid-cols-3">
                <div className="tablet:col-span-3"><h2 className="text-lg font-medium text-ink">{text.address} e mapa</h2><p className="mt-1 text-sm text-muted">O mapa prioriza latitude e longitude; o endere&ccedil;o serve como apoio.</p></div>
                ...
                <Field label="Latitude" type="number" step="any" value={data.latitude} onChange={(event) => setData('latitude', event.target.value)} /><Field label="Longitude" type="number" step="any" value={data.longitude} onChange={(event) => setData('longitude', event.target.value)} />
                <div className="tablet:col-span-3"><Map latitude={data.latitude === '' ? null : Number(data.latitude)} longitude={data.longitude === '' ? null : Number(data.longitude)} title={'Pr\u00e9via da localiza\u00e7\u00e3o'} /></div>
            </section>
