...
export default function AsyncMediaUploader({ existing = [], removed = [], data, setData, compact = false }) {
    ...
    return <section className={`rounded-xl border border-line bg-white shadow-sm ${compact ? 'p-5' : 'p-6'}`}>
        ...
        <label className={`mt-4 block cursor-pointer rounded-xl border border-dashed border-brand/40 bg-brand/5 text-center ${compact ? 'p-4' : 'p-6'}`}><span className="text-sm font-medium text-brand">Arraste ou clique para enviar</span><span className="mt-1 block text-[.65rem] text-muted">JPG, PNG, WebP, HEIC, HEIF, MP4 ou MOV. Limite: {Math.round(maxBytes / 1024 / 1024)} MB.</span><input type="file" multiple accept=".jpg,.jpeg,.png,.webp,.heic,.heif,.mp4,.mov" className="sr-only" onChange={(event) => { selectFiles(Array.from(event.target.files || [])); event.target.value = ''; }} /></label>
