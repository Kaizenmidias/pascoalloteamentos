...
function HomeHero({ slides = [] }) {
    const items = slides.length ? slides.slice(0, 5) : [{ title: HeroText.title, excerpt: HeroText.description, media_assets: [] }];
            ...
            <div className="relative min-h-[75svh]">
                ...
                <div className="relative z-10 mx-auto flex min-h-[75svh] max-w-[80rem] items-center px-5 pt-20 text-center">
                    <div className="mx-auto max-w-[980px]">
                        <h1 className="mx-auto max-w-[1000px] text-[2.125rem] font-light leading-[1.08] tracking-[-.02em] tablet:text-[3rem] desktop:text-[3.9375rem]">{HeroText.title}</h1>
                        <p className="mx-auto mt-5 max-w-[760px] text-[1.125rem] font-light leading-[1.45] text-white/90 desktop:text-[1.4375rem]">{HeroText.description}</p>
                    </div>
                </div>
...
export default function Home({ featuredItems = [], properties = [], posts = [], homeNumbers = [] }) {
    const { props } = usePage();
