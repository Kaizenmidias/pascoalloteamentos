    ...
    return (
        <section className="rounded-xl border border-line bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div><h2 className="text-lg font-medium text-ink">Atualizações mensais da obra</h2><p className="mt-1 text-sm text-muted">Agrupe as fotos por mês e ano. Atualizações sem fotos não aparecem no site.</p></div>
                <Button type="button" onClick={() => updateRows([...rows, { progress_date: '', photos: [], media_assets: [], remove_media_ids: [], media_order: [], _key: crypto.randomUUID() }])}>Adicionar atualização</Button>
            </div>
                    ...
                    return (
                        <article key={row.id || row._key || index} className="rounded-xl border border-line bg-surface p-4">
                            <div className="flex items-start justify-between gap-4">
                                ...
                                <button type="button" className="text-xs font-medium text-red-700" onClick={() => updateRows(rows.filter((_, rowIndex) => rowIndex !== index))}>Excluir atualização</button>
                            </div>
                            <div className="mt-4"><UploadDropZone label="Fotos desta atualização" description="JPG, PNG, WebP ou HEIC." accept="image/jpeg,image/png,image/webp,.heic,.heif" multiple limit={limit} onFiles={(files) => update(index, { photos: files.slice(0, limit) })} /></div>
                            {assets.length > 0 && <div className="mt-4 grid gap-3 tablet:grid-cols-3 desktop:grid-cols-4">{assets.map((asset, mediaIndex) => <div key={asset.id} className="overflow-hidden rounded-lg border border-line bg-white"><img src={asset.url} alt={asset.alt_text || ''} className="aspect-[4/3] w-full object-cover" /><div className="flex items-center justify-between gap-2 p-2 text-xs"><button type="button" disabled={mediaIndex === 0} onClick={() => moveMedia(index, mediaIndex, -1)}>Anterior</button><button type="button" disabled={mediaIndex === assets.length - 1} onClick={() => moveMedia(index, mediaIndex, 1)}>Próxima</button><button type="button" className="text-red-700" onClick={() => update(index, { remove_media_ids: [...removed, asset.id] })}>Remover</button></div></div>)}</div>}
                        </article>
                    );
                })}
            </div>
...
export default function ContentManager({ data, setData, item, showPlans = true, showStages = true, showSpecialImages = true, showFaqs = true, showDocuments = true, showFeaturedUpload = false, showSeo = true }) {
    ...
    return <>
        ...
        {showStages && <ProgressUpdatesManager data={data} setData={setData} limit={mediaLimit} />}
...
export const contentDefaults = (item, stageDefinitions = null) => ({
    ...
    floor_plans: item?.floor_plans || [], documents: item?.documents || [], construction_stages: stageDefinitions ? fixedStageDefaults(item, stageDefinitions) : (item?.construction_stages || []), progress_updates: item?.construction_progress_updates || [], faqs: item?.faqs || [],
