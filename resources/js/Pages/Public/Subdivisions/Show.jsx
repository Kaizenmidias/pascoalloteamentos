...
function Faq({ items = [] }) {
    const rows = items.filter((item) => item.is_active !== false).map((item) => [item.question, item.answer]);
    const visible = rows.length ? rows : defaultFaqs;
    return <section className="bg-surface py-14 tablet:py-16"><SectionContainer><div className="text-center"><Eyebrow>Perguntas frequentes</Eyebrow><Title>Esclare&ccedil;a suas principais d&uacute;vidas</Title></div><div className="mt-9 grid gap-x-12 tablet:grid-cols-2">{visible.map(([question, answer]) => <details key={question} className="group border-b border-line"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 py-5 text-sm font-medium text-ink"><span>{question}</span><span className="text-xl text-brand group-open:rotate-45">+</span></summary><p className="pb-5 text-sm font-light leading-6 text-muted">{answer}</p></details>)}</div></SectionContainer></section>;
}

export default function Show({ item, globalWhatsapp }) {
    const image = featuredMedia(item);
    return <PublicLayout><SeoHead title={item.seo?.title || item.title} description={item.seo?.description || item.excerpt} /><Hero item={item} image={image} globalWhatsapp={globalWhatsapp} /><About item={item} image={image} /><Features items={item.features} /><Promotions items={item.promotions} /><Documents documents={item.documents} /><Location item={item} globalWhatsapp={globalWhatsapp} /><Progress item={item} /><Gallery item={item} /><Faq items={item.faqs} /></PublicLayout>;
}
