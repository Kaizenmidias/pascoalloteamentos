...
function Hero({ item, image, globalWhatsapp }) {
    ...
    return <>
        ...
        {facts.length > 0 && <SectionContainer className="relative z-20 -mt-12"><div className="grid overflow-hidden rounded-xl bg-white p-3 shadow-[0_12px_32px_rgba(17,17,17,.1)] tablet:grid-cols-2 desktop:grid-cols-4">{facts.map(([label, value, path]) => <article key={label} className="flex min-h-24 items-center gap-4 rounded-lg border border-line px-5 py-4"><svg viewBox="0 0 24 24" className="size-8 shrink-0 fill-none stroke-brand stroke-[1.5]" strokeLinecap="round" strokeLinejoin="round"><path d={path} /></svg><div><span className="block text-[.65rem] uppercase text-muted">{label}</span><strong className="mt-1 block text-lg font-medium text-brand">{value}</strong></div></article>)}</div></SectionContainer>}
    </>;
}
...
function Location({ item, globalWhatsapp }) {
    ...
    return <section id="localizacao" className="scroll-mt-28 bg-surface py-14 tablet:py-[72px]"><SectionContainer className="grid gap-10 tablet:grid-cols-[.82fr_1.18fr] tablet:items-center"><div><Eyebrow>Localização</Eyebrow><Title>Localização estratégica para facilitar seu dia a dia</Title>{address && <p className="mt-5 max-w-xl text-base leading-7 text-muted">{address}</p>}<WhatsAppButton item={item} className="mt-7" /></div><div className="min-h-[360px] overflow-hidden rounded-xl"><Map latitude={item.latitude} longitude={item.longitude} address={address} title={`Localização de ${item.title}`} /></div></SectionContainer></section>;
}
