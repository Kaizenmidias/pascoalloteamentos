...
function MediaOrderManager({ items, setData }) {
    ...
    return <section className="rounded-xl border border-line bg-white p-6 shadow-sm"><h2 className="text-lg font-medium text-ink">Ordem da galeria</h2><p className="mt-1 text-sm text-muted">Organize imagens e videos na sequencia exibida no site.</p><div className="mt-5 grid gap-3 tablet:grid-cols-3 desktop:grid-cols-4">{items.map((asset, index) => <article key={asset.id} className="overflow-hidden rounded-xl border border-line"><div className="relative">{asset.type === 'video' || asset.mime_type?.startsWith('video/') ? <video src={asset.poster_url || asset.url} poster={asset.poster_url || undefined} controls className="aspect-[4/3] w-full bg-ink object-cover" /> : <img src={asset.url} alt={asset.alt_text || ''} className="aspect-[4/3] w-full object-cover" />}{(asset.type === 'video' || asset.mime_type?.startsWith('video/')) && <span className="absolute inset-0 grid place-items-center text-xs font-medium uppercase text-white">Video</span>}</div><div className="flex justify-between p-3 text-xs"><button type="button" disabled={index === 0} onClick={() => move(index, -1)}>Subir</button><button type="button" disabled={index === items.length - 1} onClick={() => move(index, 1)}>Descer</button></div></article>)}</div></section>;
}
...
export const contentDefaults = (item, stageDefinitions = null) => ({
    ...
    floor_plans: item?.floor_plans || [], documents: item?.documents || [], construction_stages: stageDefinitions ? fixedStageDefaults(item, stageDefinitions) : (item?.construction_stages || []), progress_updates: item?.construction_progress_updates || item?.constructionProgressUpdates || [], faqs: item?.faqs || [],
