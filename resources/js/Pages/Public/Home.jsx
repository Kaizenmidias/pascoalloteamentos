...
export default function Home({ featuredItems = [], properties = [], posts = [] }) {
    return (
        <PublicLayout>
            ...
            <section className="py-[var(--section-space)]"><Container className="grid gap-10 desktop:grid-cols-[41%_59%] desktop:gap-14"><div><p className="eyebrow">Blog</p><h2 className="section-title mt-3">Conteúdos que inspiram decisões</h2><Link href="/blog" className="mt-6 inline-block text-sm font-medium uppercase text-brand">Ver todos os artigos →</Link></div><div className="grid gap-[1.875rem] tablet:grid-cols-3">{posts.map((post) => <Link href={`/blog/${post.slug}`} key={post.id} className="group overflow-hidden rounded-card border border-line bg-white transition duration-300 hover:-translate-y-1 hover:shadow-card"><div className="overflow-hidden"><img src={post.featured_media?.url || '/reference-assets/blog-city.jpg'} alt="" className="aspect-[1/.56] w-full object-cover transition duration-500 group-hover:scale-[1.03]" /></div><div className="p-5"><h3 className="text-[1.25rem] font-normal leading-[1.2] text-ink">{post.title}</h3><span className="mt-5 block text-base font-medium uppercase text-brand">Leia mais</span></div></Link>)}</div></Container></section>
        </PublicLayout>
    );
}
